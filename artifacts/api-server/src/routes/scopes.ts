...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
  ...
  const { name, slug, sow, budgetCents, status, startDate, targetDate } =
    req.body;
...
router.patch("/:scopeId", requireAuth, async (req, res) => {
  const { name, sow, budgetCents, status, startDate, targetDate, order } =
    req.body;
  const [updated] = await db
    ...
    .where(eq(scopes.id, Number(req.params.scopeId)))
    .returning();
...
router.delete("/:scopeId", requireAuth, async (req, res) => {
  await db.delete(scopes).where(eq(scopes.id, Number(req.params.scopeId)));
